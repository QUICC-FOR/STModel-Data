...
query <- paste("SELECT ST_X(geom) as lon, ST_Y(geom) as lat, x , y, var, ", windows[i] ," as min_yr,",windows[i+1]," as max_yr, val FROM (
...
WHERE (var='bio1' OR var='bio12') AND (yr>=",windows[i]," AND yr <=",windows[i+1],") AND clim_center='",GCM_df[x,1],"' AND mod='",GCM_df[x,2],"' AND run='",GCM_df[x,3],"'  AND scenario='",GCM_df[x,4],"' AND ST_Intersects(raster,env_plots)
GROUP BY var, yr
) as pixels;",sep="")