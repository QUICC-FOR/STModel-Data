...
geom_point(aes(x = lon, y = lat),data = res_plotInfoData,colour="springgreen4",size=0.5,alpha=0.3)+
scale_x_continuous(expand=c(0,0)) + scale_y_continuous(expand=c(0,0))+ 
xlab("Longitude") + ylab("Latitude") + theme(base_size = 10,axis.title=element_text(face="bold"),axis.text = element_text(face="bold"))
